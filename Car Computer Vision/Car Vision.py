...
dashcam = cv2.VideoCapture('Car Computer Vision/WATCH_ Dashcam footage emerges of Bedfordview shoot-out.mp4')

while True:
    successframe, frame = dashcam.read()
    grayscale_vers = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    car_coordinates = trained_car_data.detectMultiScale(grayscale_vers, scaleFactor=1.2)
